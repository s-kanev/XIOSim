...
class GangAllocator : public BaseAllocator {
  public:
    ...
    ~GangAllocator();
    int AllocateCoresForProcess(int asid, std::vector<double> scaling, double serial_runtime);
...
class PenaltyAllocator : public BaseAllocator {
  ...
  private:
     ...
     */
    void FirstAllocationInit(double current_scaling_factor, double current_serial_runtiem);
...
class LocallyOptimalAllocator : public BaseAllocator {
  public:
    /* Gets a singleton allocator for a system with @ncores cores. */
    static LocallyOptimalAllocator& Get(OptimizationTarget opt_target,
                                        SpeedupModelType speedup_model_type,
                                        double core_power,
                                        double uncore_power,
                                        int num_cores) {
  ...
  private:
    LocallyOptimalAllocator(OptimizationTarget opt_target,
                            SpeedupModelType speedup_model_type,
                            double core_power,
                            double uncore_power,
                            int num_cores);
