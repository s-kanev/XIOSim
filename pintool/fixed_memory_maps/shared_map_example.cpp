...
typedef allocator<MapValueType, fixed_managed_shared_memory::segment_manager>
    MapValueAllocator;
typedef map<KeyType, ValueType, std::less<KeyType>, MapValueAllocator> ShmMap;
typedef allocator<char, fixed_managed_shared_memory::segment_manager> CharAllocator;
typedef basic_string<char, std::char_traits<char>, CharAllocator> ShmString;
typedef allocator<ShmString, fixed_managed_shared_memory::segment_manager>
    StringAllocator;
typedef allocator<void, fixed_managed_shared_memory::segment_manager> VoidAllocator;
typedef allocator<mystruct_t, fixed_managed_shared_memory::segment_manager>
    StructAllocator;
...
fixed_managed_shared_memory *global_shm;
...
void* myfunction(void *arg) {
  int value = *((int*) arg);
  VoidAllocator alloc_inst(global_shm->get_segment_manager());
  for (int i = 0; i < 100; i++) {
  ...
  switch (pid) {
    ...
    default:  { // parent
      ...
      for (int i = 0; i < mymap->size(); i ++) {
        ...
        std::cout << "Value is located at " << &mymap->at(i) << std::endl;
      }
      exit(0);
      break;
    }
