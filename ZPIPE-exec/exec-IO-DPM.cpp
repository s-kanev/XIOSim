  ...
  STQ_senior_head(0), partial_forward_throttle(false)
{
  ...
  else
  {
    ...
    if(core->memory.DL2)
      core->memory.DTLB2 = cache_create(core,name,CACHE_READONLY,sets,assoc,1,rp,'w','t','n',banks,1,latency,MSHR_entries,1,core->memory.DL2,core->memory.DL2_bus); /* on a complete TLB miss, go to the L2 cache to simulate the traffic from a HW page-table walker */
    else
      core->memory.DTLB2 = cache_create(core,name,CACHE_READONLY,sets,assoc,1,rp,'w','t','n',banks,1,latency,MSHR_entries,1,uncore->LLC,uncore->LLC_bus); /* on a complete TLB miss, go to the LLC to simulate the traffic from a HW page-table walker */
  ...
  {
    core->memory.DTLB = cache_create(core,name,CACHE_READONLY,sets,assoc,1,rp,'w','t','n',banks,1,latency,MSHR_entries,1,core->memory.DTLB2,core->memory.DTLB_bus);
...
void core_exec_IO_DPM_t::update_execution_otags(tick_t old_sim_cycle)
{
  ...
  sqword_t cycle_diff = sim_cycle - old_sim_cycle;
