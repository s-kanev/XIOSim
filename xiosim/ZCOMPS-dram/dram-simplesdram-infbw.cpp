 ...
 * __COPYRIGHT__ GT
 */
#define COMPONENT_NAME "simplesdram-infbw"
...
class dram_simplesdram_infbw_t:public dram_t
{
  protected:
  ...
  public:
  /* CREATE */
  dram_simplesdram_infbw_t(int arg_num_ranks,
                     int arg_num_banks,
                     double t_RAS,
                     double t_RCD,
                     double t_CAS,
  ...
  DRAM_ACCESS_HEADER
  {
    ...
    if(array[rank][bank].current_row == row) /* row buffer hit */
    {
      ...
      when_done = std::max(when_start + lat + time_till_avail,when_data_bus_ready + data_lat);
...
//        array[rank][bank].when_available = uncore->sim_cycle + c_WR;
  ...
  DRAM_REG_STATS_HEADER
  {
    ...
    auto& row_buffer_hits_st = stat_reg_counter(sdb, true, "dram.row_buffer_hits",
                                                "total number of accesses that hit in row buffer",
                                                &row_buffer_hits, 0, TRUE, NULL);
    auto total_access_st = stat_find_stat<int>(sdb, "dram.total_access");
    assert(total_access_st != nullptr);
    stat_reg_formula(sdb, true, "dram.row_buffer_hit_rate",
                     "fraction of accesses that hit in row buffer",
                     row_buffer_hits_st / *total_access_st, NULL);
  }
