...
*/
  ...
  if(!strcasecmp(exec_opt_string,"IO-DPM"))
    return std::make_unique<class core_exec_IO_DPM_t>(core);
#else
...
core_exec_IO_DPM_t::~core_exec_IO_DPM_t() {
...
void core_exec_IO_DPM_t::reg_stats(xiosim::stats::StatsDatabase* sdb) {
    int coreID = core->id;
    ...
    stat_reg_core_formula(sdb, true, coreID, "exec_avg_snatced", "average snatch-backs per uop",
    ...
    stat_reg_core_counter(sdb, true, coreID, "load_nukes",
                     "num pipeflushes due to load-store order violation", &core->stat.load_nukes, 0,
                     true, NULL);
    stat_reg_core_counter(sdb, true, coreID, "wp_load_nukes",
    ...
    for (int i = 0; i < core->knobs->exec.num_exec_ports; i++) {
        ...
        auto& issue_occupancy_st =
                stat_reg_core_counter(sdb, true, coreID, buf, "total issue occupancy",
                                      &port[i].issue_occupancy, 0, true, NULL);
        sprintf(buf, "port%d_occ_avg", i);
        stat_reg_core_formula(sdb, true, coreID, buf, "average issue occupancy",
                              issue_occupancy_st / *sim_cycle_st, NULL);
    }

    stat_reg_core_counter(sdb, true, coreID, "int_FU_occupancy", "int FUs occupancy",
                     &core->stat.int_FU_occupancy, 0, true, NULL);
    stat_reg_core_counter(sdb, true, coreID, "fp_FU_occupancy", "fp FUs occupancy",
                     &core->stat.fp_FU_occupancy, 0, true, NULL);
    stat_reg_core_counter(sdb, true, coreID, "mul_FU_occupancy", "mul FUs occupancy",
...
void core_exec_IO_DPM_t::DL1_callback(void * const op)
{
  ...
  class core_exec_IO_DPM_t * E = (core_exec_IO_DPM_t*)uop->core->exec.get();
...
void core_exec_IO_DPM_t::DTLB_callback(void * const op)
{
  struct uop_t * uop = (struct uop_t*) op;
  struct core_t * core = uop->core;
  struct core_exec_IO_DPM_t * E = (core_exec_IO_DPM_t*)uop->core->exec.get();
  if(uop->alloc.LDQ_index != -1)
  {
    struct LDQ_t * LDQ_item = &E->LDQ[uop->alloc.LDQ_index];
...
void core_exec_IO_DPM_t::load_miss_reschedule(void * const op, const int new_pred_latency)
{
  struct uop_t * uop = (struct uop_t*) op;
  struct core_t * core = uop->core;
  struct core_exec_IO_DPM_t * E = (core_exec_IO_DPM_t*)core->exec.get();
  struct core_knobs_t * knobs = core->knobs;
          ...
          if(check_load_issue_conditions(LDQ[index].uop)) /* retval of true means load is predicted to be cleared for issue */
          {
            ...
            if(!LDQ[index].first_byte_requested)
            {
              if((cache_enqueuable(core->memory.DTLB.get(), asid, memory::page_table_address(asid, uop->oracle.virt_addr))) &&
                 (!send_to_dl1 || (send_to_dl1 && cache_enqueuable(core->memory.DL1.get(), asid, uop->oracle.virt_addr))) &&
                 ...
                 (port[uop->alloc.port_assignment].STQ->pipe[0].uop == NULL))
              {
                ...
                if(!uop->oracle.is_sync_op && uop->exec.when_addr_translated == TICK_T_MAX) {
                ...
                if(uop->oracle.is_sync_op ||
                   cache_single_line_access(core->memory.DL1.get(), uop->oracle.virt_addr, uop->decode.mem_size))
                {
...
//                          insert_ready_uop(odep->uop);
            ...
            if(LDQ[index].first_byte_requested && !LDQ[index].last_byte_requested && !uop->oracle.is_sync_op)
            {
              ...
              if((!send_to_dl1 || (send_to_dl1 && cache_enqueuable(core->memory.DL1.get(), asid, split_addr))) &&
                 (!uop->oracle.is_repeated || (uop->oracle.is_repeated && core->memory.mem_repeater->enqueuable(CACHE_READ, asid, split_addr))))
              {
                if(send_to_dl1) {
                  ...
                  cache_enqueue(core, core->memory.DL1.get(), NULL, CACHE_READ, asid, uop->Mop->fetch.PC, split_addr, uop->exec.action_id, 0, NO_MSHR, uop, DL1_split_callback, load_miss_reschedule, translated_callback, get_uop_action_id, true);
                }
                if(uop->oracle.is_repeated) {
...
bool core_exec_IO_DPM_t::STQ_deallocate_std(struct uop_t * const uop)
{
  ...
  if(send_to_dl1 && !cache_enqueuable(core->memory.DL1.get(), asid, uop->oracle.virt_addr))
    return false;
  ...
  if(STQ[STQ_head].first_byte_requested && !STQ[STQ_head].last_byte_requested)
  {
    ...
    /* Wait until we can submit to DL1 */
    if(send_to_dl1 && !cache_enqueuable(core->memory.DL1.get(), asid, split_addr))
      return false;
    /* Wait until we can submit to repeater */
    if(uop->oracle.is_repeated)
    ...
    if(send_to_dl1) {
      ...
      cache_enqueue(core, core->memory.DL1.get(), NULL, CACHE_WRITE, asid, uop->Mop->fetch.PC, split_addr, dl1_split_uop->exec.action_id, 0, NO_MSHR, dl1_split_uop, store_dl1_split_callback, NULL, store_translated_callback, get_uop_action_id);
    }
     ...
     if(uop)
      {
        if(uop->decode.fusion_next)
          printf(" %s \t|", print_uop(uop->decode.fusion_next));
        else
          printf(" %s \t|", print_uop(uop));
      }
      else
        printf(" \t|");
    }
...
/* when_completed should be already assigned (and reliable since only loads can have variable latencies, but they are already processed) */
             ...
             else
             {
                 ...
                 {
                    int curr_fp_penalty = get_fp_penalty(curr_uop);
...
bool core_exec_IO_DPM_t::exec_fused_ST(struct uop_t * const uop)
{
  ...
  zesto_assert((curr_uop->alloc.STQ_index >= 0) && (curr_uop->alloc.STQ_index < core->knobs->exec.STQ_size), false);
