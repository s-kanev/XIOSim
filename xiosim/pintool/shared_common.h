...
template <typename K, typename V> class SharedMemoryMapCommon {
  protected:
    ...
    typedef boost::interprocess::allocator<MapValueType, managed_shared_memory::segment_manager>
        MapValueAllocator;
    typedef boost::interprocess::map<K, V, std::less<K>, MapValueAllocator> InternalMap;
  ...
  public:
    ...
    SharedMemoryMapCommon(managed_shared_memory* shm, const char* internal_data_name)
    ...
    // not exist, a value is default-constructed, inserted into the map, and
    ...
    typename InternalMap::iterator find(K key) { return internal_map->find(key); }

    std::size_t erase(const K& key) { return internal_map->erase(key); }
  ...
  private:
    void initialize() {
        ...
        MapValueAllocator alloc_inst(shm->get_segment_manager());
        internal_map =
            shm->find_or_construct<InternalMap>(data_key.c_str())(std::less<K>(), alloc_inst);
    }

    virtual V& access_operator(const K& key) = 0;

    // Returns a std::pair<size_t, size_t> tuple. The first element is the
    // total size of the shared memory segment, and the second element is
    ...
    // TODO: This must be synchronized so that no other process is mapping
    // the segment while this operation takes place.
    void check_and_grow_shared_memory() {
        // For now, growing shared memory is really messy. Let's just make
        // the segment big enough so that we don't ever need to.
        assert(false);

// But if we wanted to grow memory, the code would look something
// like this.
#if 0
            ...
            assert(shm_state.first + DEFAULT_GROW_MEMORY_SIZE ==
                   new_shm_state.first);
#endif
    }
};
