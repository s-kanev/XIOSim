 ...
 * allocation.
 */
enum OptimizationTarget { ENERGY, THROUGHPUT };

/* The different types of speedup models that can be created. */
enum SpeedupModelType { LINEAR, LOGARITHMIC };
...
class BaseSpeedupModel {
  public:
     ...
     * compute energy.
     */
    BaseSpeedupModel(double core_power,
                     double uncore_power,
                     int num_cores,
     ...
     *   return value.
     */
    virtual void OptimizeEnergy(std::map<int, int>& core_allocs,
     ...
     *   See OptimizeEnergy().
     */
    virtual void OptimizeThroughput(std::map<int, int>& core_allocs,
                                    std::vector<double>& process_scaling,
                                    std::vector<double>& process_serial_runtime) = 0;
     ...
     */
    virtual double ComputeScalingFactor(std::vector<double>& process_scaling) = 0;
     ...
     */
    void OptimizeForTarget(std::map<int, int>& core_allocs,
                           std::vector<double>& process_scaling,
  ...
  protected:
     ...
     */
    virtual double
    ComputeRuntime(int num_cores_alloc, double process_scaling, double process_serial_runtime) = 0;
     ...
     */
    double ComputeSpeedup(int core_alloc, double process_scaling, double process_serial_runtime);
     ...
     */
    double ComputeEnergy(std::map<int, int>& core_alloc,
                         std::vector<double>& process_scaling,
                         std::vector<double>& process_serial_runtime);
     ...
     */
    double ComputeThroughputMetric(std::map<int, int>& core_alloc,
                                   std::vector<double>& process_scaling,
                                   std::vector<double>& process_serial_runtime);
  ...
  private:
     ...
     */
    typedef double (BaseSpeedupModel::*metric_function_t)(std::map<int, int>&,
                                                          std::vector<double>&,
                                                          std::vector<double>&);
