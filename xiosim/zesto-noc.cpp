...
std::unique_ptr<struct bus_t> bus_create(const char* const name,
                                         const int width,
                                         const tick_t* clock,
                                         const int ratio) {
    auto bus = std::make_unique<struct bus_t> ();
    bus->name = name;
    bus->width = width;
    bus->clock = clock;
    bus->ratio = ratio;
...
void bus_reg_stats(xiosim::stats::StatsDatabase* sdb,
    ...
    auto& accesses_st = stat_reg_comp_counter(sdb, true, core_id, bus->name.c_str(), "accesses",
                                              "number of accesses to bus %s",
                                              &bus->stat.accesses, 0, true, NULL);

    auto& utilization_st = stat_reg_comp_double(sdb, true, core_id, bus->name.c_str(), "utilization",
                                                "cumulative cycles of utilization of bus %s",
                                                &bus->stat.utilization, 0, true, NULL);
    stat_reg_comp_formula(sdb, true, core_id, bus->name.c_str(), "avg_burst",
                          "avg cycles utilized per transfer of bus %s",
                          utilization_st / accesses_st, "%12.4f");

    stat_reg_comp_formula(sdb, true, core_id, bus->name.c_str(), "duty_cycle",
                          "fraction of time bus %s was in use",
                          utilization_st / *sim_cycle_st, "%12.4f");

    auto& pf_utilization_st =
            stat_reg_comp_double(sdb, true, core_id, bus->name.c_str(), "pf_utilization",
                                 "cumulative cycles of utilization of bus %s for prefetches",
                                 &bus->stat.prefetch_utilization, 0, true, NULL);
    stat_reg_comp_formula(sdb, true, core_id, bus->name.c_str(), "pf_duty_cycle",
...
int bus_free(const struct bus_t* const bus) {
    ...
    if (uncore_knobs.fsb_magic && bus->name == "FSB")
        return true;
