...
void bus_reg_stats(xiosim::stats::StatsDatabase* sdb,
    ...
    auto& accesses_st = stat_reg_comp_counter(sdb,
                                              true,
                                              core_id,
                                              bus->name,
                                              "accesses",
    ...
    auto& utilization_st = stat_reg_comp_double(sdb,
                                                true,
                                                core_id,
                                                bus->name,
                                                "utilization",
                                                "cumulative cycles of utilization of bus %s",
                                                &bus->stat.utilization,
                                                0,
                                                TRUE,
                                                NULL);
    stat_reg_comp_formula(sdb,
                          true,
                          core_id,
                          bus->name,
                          "avg_burst",
                          "avg cycles utilized per transfer of bus %s",
                          utilization_st / accesses_st,
                          "%12.4f");
    ...
    stat_reg_comp_formula(sdb,
                          ...
                          utilization_st / *sim_cycle_st,
                          "%12.4f");

    auto& pf_utilization_st =
            stat_reg_comp_double(sdb,
                                 true,
                                 core_id,
                                 bus->name,
                                 "pf_utilization",
                                 "cumulative cycles of utilization of bus %s for prefetches",
                                 &bus->stat.prefetch_utilization,
                                 0,
                                 TRUE,
                                 NULL);
    stat_reg_comp_formula(sdb,
                          ...
                          pf_utilization_st / *sim_cycle_st,
                          "%12.4f");
}
